...
| **LEAP tab** (in OpportunitiesCard, commit f2bd329) | `scripts/leap_scanner_uw.py --json`, `data/leap.json`, `app/api/leap/route.ts`, `useLeap.ts` | GET-only route reads `data/leap.json`; returns `{results: []}` empty state. POST-trigger deferred — until then, ssh `radon@ib-gateway` and run `python3 scripts/leap_scanner_uw.py --preset megacap --json`. Script mirrors output to both `reports/leap-scan-uw.json` and `data/leap.json`. Top 5 by `best_gap`. Empty state literally embeds the CLI command. |
